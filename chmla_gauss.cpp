...
bool gauss_elimination(rmatrix& A, rvector& b) {
	int n = A.size();
	bool change_sign = 0; // Змінна для зміни знаку визначника
	...
	for (int k = 0; k < n; ++k) {
		...
		if (imax != k) change_sign++; // Якщо виконали перестановку
	...
	if (change_sign) {
		det = -det; // Зміна знаку при непарній кількості перестановок стовпців
	}
...
void test() {
	...
	for (int i = 0; i < 4; ++i) {
		...
		if (!x.empty()) {
			...
			cout << "A is singular, it has infinitely many solutions" << endl;
		}
